  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <input name="phone" placeholder="Phone Number" value={form.phone} onChange={handleChange} required className={inputClass} />
      </div>
      <input name="addressLine1" placeholder="Address Line 1" value={form.addressLine1} onChange={handleChange} required className={inputClass} />
      <input name="addressLine2" placeholder="Address Line 2 (Optional)" value={form.addressLine2} onChange={handleChange} className={inputClass} />
      ...
      <div className="flex items-center gap-4 pt-2">
        <button type="submit" disabled={saving} className="px-6 py-3 rounded-xl bg-[#0f2c59] text-white text-sm font-bold hover:bg-blue-800 disabled:opacity-50 transition-all flex items-center gap-2">
          {saving ? <LuLoader size={16} className="animate-spin" /> : <LuCheck size={16} />}
          Save Address
        </button>
        {onCancel && (
          <button type="button" onClick={onCancel} className="px-6 py-3 rounded-xl border border-gray-200 text-sm font-bold text-gray-600 hover:bg-gray-50 transition-all">
            Cancel
          </button>
        )}
      </div>
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (!buyNowItem) {
        ...
        finalCartItems = latestCartJson ? JSON.parse(latestCartJson) : reduxCartItems;
      }

      const payload = {
        items: finalCartItems.map((item) => ({ productId: item.id, quantity: item.quantity })),
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                  <LuMapPin size={18} className="text-[#0f2c59]" /> Shipping Address
                </h2>
                {addresses.length > 0 && (
                  <button onClick={() => setShowAddressForm(!showAddressForm)} className="text-sm font-bold text-[#0f2c59] hover:underline flex items-center gap-1">
              ...
              {addressLoading ? (
                ...
                <AddressForm onSave={(data) => createAddressMutation.mutate(data)} onCancel={addresses.length > 0 ? () => setShowAddressForm(false) : null} saving={createAddressMutation.isPending} />
              ) : (
                <div className="space-y-3">
                  {addresses.map((addr) => (
                    <label key={addr.id} className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${selectedAddressId === addr.id ? "border-[#0f2c59] bg-blue-50/30" : "border-gray-100 hover:border-gray-200"}`}>
                      <input type="radio" name="address" checked={selectedAddressId === addr.id} onChange={() => setSelectedAddressId(addr.id)} className="mt-1 accent-[#0f2c59]" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-gray-900">{addr.name} <span className="font-normal text-gray-500">| {addr.phone}</span></p>
                        <p className="text-sm text-gray-600 mt-1">
                          {addr.addressLine1}{addr.addressLine2 ? `, ${addr.addressLine2}` : ""}, {addr.city}, {addr.state} - {addr.pincode}
                        </p>
                        {addr.isDefault && <span className="inline-block mt-1 text-xs font-bold text-[#0f2c59] bg-blue-50 px-2 py-0.5 rounded-full">Default</span>}
                      </div>
                    </label>
                  ))}
                </div>
